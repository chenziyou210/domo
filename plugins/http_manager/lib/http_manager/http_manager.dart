...
import 'package:star_common/common/crypto/xxtea.dart';
...
class HttpManager {
  ...
  void updateHost({String? host = null,String port = ""}) {
    this._config=HttpConfig.clone(this._config!,host: host,port: port);
  }
...
class _WrapDio {
  ...
  Future<_DioResultContainer> request(HttpContainer container,
    ...
    if(isEncrypt == "1") {
      //1.rsa解密出随机字符串
      final privatePem = await rootBundle.loadString('assets/rsa/private.pem');
      final privateKey = RSAKeyParser().parse(privatePem) as RSAPrivateKey;
      final decrypt = Encrypter(RSA(privateKey: privateKey));
      String encryptedStr = response.headers.map['client-iv']?.first ?? '';
      String decrypted = decrypt.decrypt(Encrypted.fromBase64(encryptedStr));
      //2.随机字符串拼接得到xxtea的key
      String key = '${decrypted.substring(0,5)}$encryptSplice${decrypted.substring(5)}';
      //3.解密data
      data  = xxtea.decryptToString(data, key);
      data = json.decode(data);
      print('解密的数据:$data');
    }

    _DioResultContainer result = _DioResultContainer(data: data, code: response.statusCode);
